...
channel_length = 2e-6 		#(20 nm) 

extension_length = 2e-6 		#(20 nm) 

bulk_height = 5e-5 		#(500 nm) 

oxide_thickness = 2e-7 		#(2 nm) 

source_contact = 4e-6		#(40 nm) 

drain_contact = 4e-6		#(40 nm) 
...
create_gmsh_mesh(file="comb3.msh", mesh="mos2d") # Load mesh file
...
csvfile = open("ramp_currents_full.csv", "w", newline='') # Open CSV file
writer = csv.writer(csvfile)  # Create CSV writer

header = ["Step"]  # Init header
for name in ordered_contacts: # Loop contacts
    header += [f"{name}_V", f"{name}_I"]  # Add V and I columns
header += ["gate_V"]  # Add gate V column
writer.writerow(header)  # Write header

# Sweep gate to capture properly scaled transfer characteristic
rampbias(device, "gate", 3, 0.01, 1e-12, 100, 1e5, 5e13, CSVCallback) # Ramp gate to 3V with logging
write_devices(file="gmsh_mos2d", type="vtk") # Export ON-state VTK

csvfile.close() # Close CSV file

#==================================================================================
# OUTPUT CHARACTERISTICS FOR NMOS (Uncomment to test Velocity Saturation)
#==================================================================================
# print("Running Output Characteristics...") # Print status
# csvfile_out = open("ramp_output_curves.csv", "w", newline='') # Open output CSV
# writer = csv.writer(csvfile_out) # Create writer
# writer.writerow(header) # Write header

# rampbias(device, "gate", 0, 0.01, 1e-12, 100, 1e5, 5e13, EmptyCallback) # Reset gate
# rampbias(device, "drain", 0, 0.01, 1e-12, 100, 1e5, 5e13, EmptyCallback) # Reset drain

# # Vg = 3V Sweep
# rampbias(device, "gate", 3, 0.01, 1e-12, 100, 1e5, 5e13, EmptyCallback) # Set gate to 3V
# rampbias(device, "drain", 5, 0.05, 1e-12, 100, 1e5, 5e13, CSVCallback) # Ramp drain to 5V

# csvfile_out.close() # Close output CSV
...
class Logger(object): # Define custom logger
    def __init__(self): # Initialize logger
        ...
        self.log = open("Transfer_simulation_log.log", "a") # Open log file
